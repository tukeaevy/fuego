...
public:
  ...
  virtual void getOutputAvailability(std::vector<uint64_t>&& amounts, uint64_t minRingSize, std::vector<COMMAND_RPC_GET_OUTPUT_AVAILABILITY::output_availability>& outputs, uint64_t& recommendedRingSize, std::string& privacyLevel, const Callback& callback) = 0;
};
